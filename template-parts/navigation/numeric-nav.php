...
if ( get_previous_posts_link() ) {
    $prev_link = sprintf(
		'<li>%1s</li>',
		get_previous_posts_link( '<span class="prev-page" rel="prev"><span class="icon-left"></span> Previous</span>' )
	);
} else {
    $prev_link = '';
}
...
if ( get_next_posts_link() ) {
    $next_link = sprintf(
		'<li>%1s</li>',
		get_next_posts_link( '<span class="next-page" rel="next">Next <span class="icon-right"></span></span>' )
	);
} else {
    $next_link = '';
}
...
$label = post_type_archive_title( '', false ) . __( ' Page: ', 'affp-theme' );
